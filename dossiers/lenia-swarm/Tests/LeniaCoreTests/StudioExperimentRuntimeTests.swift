...
final class StudioExperimentRuntimeTests: XCTestCase {
    ...
    func testSandboxDiagnosticsExposeAllCausalStages() async {
        let runtime = FlowSandboxRuntime(
            params: experimentParams(),
            gridPreset: .compact128,
            initialStamp: experimentStamp(),
        ...
        eval(
            diagnostics.field,
            diagnostics.neighborSum,
            diagnostics.growthField,
            diagnostics.kernel
        )

        XCTAssertEqual(diagnostics.field.shape, [128, 128])
        XCTAssertEqual(diagnostics.neighborSum.shape, [128, 128])
        XCTAssertEqual(diagnostics.growthField.shape, [128, 128])
        XCTAssertEqual(diagnostics.kernel.shape, [128, 128])
        XCTAssertEqual(diagnostics.kernelCount, 1)
    }
}
