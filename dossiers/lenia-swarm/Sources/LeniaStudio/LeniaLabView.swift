...
final class LeniaLabModel: ObservableObject {
    ...
    @Published var causalFrame: StudioCausalFrame?
    ...
    func reset() {
        ...
        enqueueRuntimeMutation(named: "reset", runtime: runtime, generation: generation) { [weak self] in
            ...
            await self.updateCausalFrame(runtime: runtime, generation: generation)
            guard self.ownsRuntime(runtime, generation: generation) else { return }
            if self.isRunning {
                await runtime.resume()
            } else {
    ...
    func captureSpecimen(
        ...
        history.record(
            kind: .capture,
            summary: "Captured \(result.creature.name)",
            step: snapshot.step,
            details: ["fingerprint": result.component.fingerprint]
        )
        experimentStatusMessage = "Saved \(result.creature.name)"
        return result
    }
    ...
    private func initializeExperiment(
        ...
        syncHistoryState()
        await updateCausalFrame(runtime: runtime, generation: generation)
        guard ownsRuntime(runtime, generation: generation) else { return }
    }
    ...
    private func updateCausalFrame(
        runtime: LabRuntimeHandle,
        generation: Int
    ) async {
        ...
        let snapshot = await runtime.materializeStateSnapshot()
        guard ownsRuntime(runtime, generation: generation) else { return }
        let step = snapshot?.step ?? latestStep
        let rendered = await Task.detached(priority: .utility) {
            StudioCausalFrame(step: step, diagnostics: diagnostics)
        }.value
        guard ownsRuntime(runtime, generation: generation) else { return }
        causalFrame = rendered
    }
        ...
        strokeFlushTask = Task { [weak self] in
            ...
            if let batch = lastAppliedBatch {
                ...
                if let snapshot {
                    ...
                    self.syncHistoryState()
                    await self.updateCausalFrame(runtime: batch.runtime, generation: generation)
                    guard self.ownsRuntime(batch.runtime, generation: generation) else { return }
                }
            }
...
struct LeniaLabView: View {
    ...
    private var inspectorSurface: some View {
        VStack(spacing: 8) {
            ...
            case .signals:
                telemetrySurface
            case .causal:
                StudioCausalMicroscopeView(
                    frame: model.causalFrame,
                    compact: true,
                    isLoading: !model.hasSnapshot,
                    onRefresh: model.refreshCausalMicroscope,
                    refreshDisabled: !model.hasSnapshot || model.externalReplayTitle != nil
                )
            }
        }
    }
    ...
    private var inspectorContextSummary: String {
        ...
        case .causal:
            return model.causalFrame.map { "t\($0.step) · \($0.kernelCount)k" } ?? "Awaiting fields"
        }
    }
                    ...
                    if let selectedTrack1Config {
                        DisclosureGroup("Details") {
                            LabInfoSection(title: "Selected lineage") {
                                if let flow = selectedTrack1Config.flowClassification {
                                    LabCompactKeyValueRow(label: "Family", value: flow.family)
                                    LabCompactKeyValueRow(label: "Body plan", value: flow.bodyPlan)
                                    LabCompactKeyValueRow(label: "Lineage", value: flow.lineage)
                                    LabCompactKeyValueRow(label: "Specimen", value: flow.specimen)
                                } else {
        ...
        Button(action: onObserve) {
            HStack(spacing: 9) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(config.flowClassification?.bodyPlan ?? config.displayName)
                        .font(StudioType.labelStrong)
                        .foregroundStyle(StudioPalette.ink)
                        .lineLimit(2)
                    Text(
                        config.flowClassification?.lineage
                            ?? "\(config.family) / \(config.genus)"
                    )
                        .font(StudioType.dataSmall)
                        .foregroundStyle(StudioPalette.mutedInk)
                        .lineLimit(1)
                    HStack(spacing: 5) {
                        Text(config.flowClassification?.specimen ?? config.patternID)
                            .lineLimit(1)
                            .truncationMode(.middle)
                        Text(runtimeLabel)
                            .foregroundStyle(
