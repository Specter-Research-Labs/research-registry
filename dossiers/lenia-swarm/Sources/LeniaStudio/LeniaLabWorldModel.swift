...
private func featuredLabMissionPreset(config: Track1TaxonomyConfig) throws -> LabMissionPreset {
    guard let descriptor = config.featuredDescriptor else {
        throw ConfigError.invalidConfig("Featured organism has no descriptor: \(config.path)")
    }
    ...
    return LabMissionPreset(
        ...
        detail: "\(descriptor.note). Exact \(config.patternID) state and rule from the named Lenia catalog.",
        entry: entry,
