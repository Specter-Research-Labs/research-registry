...
struct StudioMetricPill: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, style == .console ? 7 : 8)
        .frame(maxWidth: .infinity, minHeight: style == .console ? 44 : 48, alignment: .leading)
        .background(
            RoundedRectangle(cornerRadius: style.cornerRadius, style: .continuous)
                .fill(
                    style == .console
                        ? StudioPalette.consoleControl.opacity(0.56)
                        : StudioPalette.surfaceSoft.opacity(0.54)
                )
        )
        .overlay {
            RoundedRectangle(cornerRadius: style.cornerRadius, style: .continuous)
                .stroke(StudioPalette.hairline.opacity(style == .console ? 0.5 : 0.34), lineWidth: 0.5)
        }
    }
}
