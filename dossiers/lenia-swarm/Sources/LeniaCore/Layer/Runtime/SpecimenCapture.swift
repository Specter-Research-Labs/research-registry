...
public func captureSpecimenComponent(
    ...
    // Parameter fields describe the local medium, not only cells whose mass is
    // part of the selected component. Preserve the complete padded neighborhood
    // so a captured specimen retains its local parameter context.
    if snapshot.parameterCount > 0 {
        for localX in 0..<captureWidth {
            let rawX = minX + localX
            for localY in 0..<captureHeight {
                let rawY = minY + localY
                if !wraps, (rawX < 0 || rawX >= width || rawY < 0 || rawY >= height) {
                    continue
                }
                let sourceX = positiveModulo(rawX, width)
                let sourceY = positiveModulo(rawY, height)
                let sourceCell = sourceX * height + sourceY
