"""
Generate chimeric (mixed cell-type) data for aggregation analysis.

Tests the paper's core "Algotype" claim: do cells of the same algorithm type
cluster together during sorting, even though sorting doesn't require this?

Produces directories:
- cell_type_aggregation_random_dist_100_tests (bubble + selection)
- cell_type_aggregation_random_dist_100_tests_bubble_insertion
- cell_type_aggregation_random_dist_100_tests_selection_insertion
- cell_type_aggregation_random_dist_100_tests_bubble_selection_insertion

Each contains exp_{i}.npy files with cell_types snapshots.
"""
...
def run_chimeric_experiment(sorting_list, cell_types_to_use, max_steps=15000):
    ...
    return status_probe.cell_types, status_probe.sorting_steps
...
def main(argv):
    ...
    print(f"Generating chimeric data: {num_experiments} experiments with {num_cells} cells each")
    ...
    for dir_name, cell_types in configs:
        ...
        for i in range(num_experiments):
            ...
            cell_types_data, sorting_steps = run_chimeric_experiment(
                sorting_list, cell_types
            )

            np.save(target_dir / f"exp_{i}.npy", np.array(cell_types_data, dtype=object))

        print(f"  Saved {num_experiments} experiments to {dir_name}/")
