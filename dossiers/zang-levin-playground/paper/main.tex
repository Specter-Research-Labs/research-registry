...
the paper's main evidential core. By contrast, we frame the K-computation surface as
$K_{\mathrm{state}}$: an exact reachable-state proposal null that preserves the state-space scale
and matched-cost contract, while omitting operator-graph traversal geometry. Across the paper, the main lesson is that claims about robustness
...
class. Adjacent+routing also fails immediately, showing that frozen-target rerouting is not
sufficient under adjacent-only execution. The reason is structural:
under immovable 1D barriers, adjacent active-active moves cannot exchange values across frozen
indices, so many paired trials are unreachable for any such policy. The adjacent variants remain
genuinely local in every condition (weighted mean swap span $= 1.00$), but they fail after
expending far more work. At frozen$=3$, adjacent+routing averages $14{,}917$ swaps and
adjacent+stubborn averages $8{,}238$, versus $91$ for long-range+routing. Under the smaller movable
control, long-range+routing retains limited success at one frozen cell ($3/15$), while the other
three variants are $0/15$ as soon as any frozen cells are introduced.
...
This $K_{\mathrm{state}}$ surface is exact for an explicit state-space null while remaining tied to
explicit operator families, explicit frozen constraints, and a distance-weighted cost model.
It should be read as a scalable state-space proxy for the formal K target, not as a direct
operator-walk hitting-time calculation. Within the long-range operator class, rerouting preserves
search efficiency under frozen barriers; within the adjacent class, the barrier semantics can
eliminate reachability altogether.
...
The factorization result is the cleanest result here. Under immovable barriers, long-range reach
without frozen-target rerouting is not enough, and frozen-target rerouting is not sufficient under
adjacent-only execution; only the combination survives. The structural reason for the adjacent
failure is that adjacent active-active moves cannot exchange values across immovable frozen
indices. The rerouting ablation then isolates the policy half of the story within the published
long-range Selection operator class.
Bubble and Insertion fail for combinatorial reasons once adjacent transpositions are blocked by
hard barriers.
...
The $K_{\mathrm{state}}$ analysis translates the operator-and-constraint story into a graded efficiency metric without
hiding behind a fixed blind
baseline. Because the state-space null is recomputed under each frozen pattern, long-range
Selection's $K_{\mathrm{state}}$ now falls from $30.44$ to $18.01$ as the reachable state space contracts, rather than
...
and leave full operator-walk K to small-$n$ or separately bounded analyses.
...
policy-aligned exploratory diagnostic. K-computation, by contrast, is framed as
$K_{\mathrm{state}}$, a matched state-space analysis rather than a descriptive fixed-baseline curve.
