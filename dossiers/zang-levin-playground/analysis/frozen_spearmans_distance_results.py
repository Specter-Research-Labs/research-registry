import numpy as np
from utils import get_monotonicity, parse_seed, frozen_path, original_records_path
...
def plot_bar_chart_for_frozen_affect(fozen_cell_number):
    success_results_bubble = get_average_final_spearman_distance(original_records_path('bubble', fozen_cell_number, SEED))
    success_results_cell_bubble = get_average_final_spearman_distance(frozen_path('bubble', fozen_cell_number, SEED))
    success_results_insertion = get_average_final_spearman_distance(original_records_path('insertion', fozen_cell_number, SEED))
    success_results_cell_insertion = get_average_final_spearman_distance(frozen_path('insertion', fozen_cell_number, SEED))
    success_results_selection = get_average_final_spearman_distance(original_records_path('selection', fozen_cell_number, SEED))
    success_results_cell_selection = get_average_final_spearman_distance(frozen_path('selection', fozen_cell_number, SEED))
...
def compare_algorithms(algo, frozen_cells):
    original_file = original_records_path(algo, frozen_cells, SEED)
    cell_file = frozen_path(algo, frozen_cells, SEED)
...
def get_observe_matrix(algo):
    return [[get_final_monotonicity(a) for a in get_cell_exp_monotonicities(frozen_path(algo, f, SEED))] for f in range(1, 4)]
