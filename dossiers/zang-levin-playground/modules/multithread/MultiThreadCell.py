...
class MultiThreadCell(threading.Thread):
    def __init__(
        ...
        self.cell_type_dict = {
            ...
            'TrueRandomBubble': 8,
            'LeftBiasedBubble': 9,
            'RightBiasedBubble': 10,
            'LowContentionBubble': 11,
            'HighContentionBubble': 12,
            'LowContentionGnome': 13,
            'HighContentionGnome': 14,
            'StubbornSelection': 15,
            'ImmovableBubble': 16,
            'MovableSelection': 17,
            'NudgeableFrozenBubble': 18,
            'NudgeableFrozenSelection': 19,
            'BiasedBubble': 20,
            'InertiaBubble': 21,
            'RandomSwap': 22,
            'AdjacentSelection': 23,
            'AdjacentStubbornSelection': 24,
            'DelayedGnome': 25,
            'BubbleClone': 26,
            'GnomeClone': 27,
        }
    ...
    def swap(self, target_position, skip_stats=False):
        ...
        if not skip_stats:
            ...
            self.status_probe.record_swap_extended(
                self, current_cell_at_target,
                current_cell_at_target.target_position, self.target_position
            )
