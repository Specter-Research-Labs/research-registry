...
A test that would tighten the bet considerably is whether morphospace position predicts behavior the descriptor never sees. Cool, Hartl, Levin and Petti's recent agnosiophobia paper ([arXiv:2605.30708](https://arxiv.org/abs/2605.30708)) sweeps a persistent 3x3 occluded patch across every nonzero pixel of four Chan-catalog creatures and finds that fragility and navigational capacity are coupled, with reorientation zones flanking lethal ones along the direction of travel, but they only run four creatures and explicitly leave open the question of why some rulesets produce coupling between perturbation and heading change while others do not. The compendium gives us a population to ask that at, by running an occlusion intervention across our existing creatures, scoring each on perturbation-heading coupling, and checking whether the score is a coordinate function on the morphospace we built without reference to it, and if it is then morphospace position is predicting a behavioral property the descriptor was never measured against, which is a stronger claim about the space than anything internal to the descriptor could make.
