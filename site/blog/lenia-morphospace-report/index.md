...
Creature discovery and morphospace analysis share the same compendium but ask different things. MAP-Elites, QD, presets, and seed libraries produce organisms that hold together while moving. The morphospace work keeps the genotype-to-phenotype relation explicit, measures loops and transport residue, and compares the resulting cloud with [EmbryoMaker morphology snapshots](https://github.com/HugoCanoFernandez/Morphospace_exploration_EMaker) and [Dryad fish body outlines](https://datadryad.org/dataset/doi:10.5061/dryad.n2z34tn2t).

Two biological cohorts are not enough to claim recurrence. The immediate biological question is narrower: does rule-layer residue correspond to hidden state in tissue, such as bioelectric voltage or transmembrane signaling, that a shape descriptor misses?

Cool, Hartl, Levin and Petti's agnosiophobia paper ([arXiv:2605.30708](https://arxiv.org/abs/2605.30708)) places a persistent 3x3 occluded patch at every nonzero pixel of four Chan-catalog creatures. Three swerve; one dies on contact. They could not predict the difference from the rules alone. We can run that intervention over the compendium and ask whether the swervers occupy one connected region of the map. That would test whether a morphospace built from morphology predicts a behavior it was never given.

Drop obstacles into the world that the creature cannot see, watch which creatures swerve and which die on contact, and ask whether the swervers sit near each other on our map. Cool et al. 2026 ran exactly that experiment and found that three of four Chan creatures swerve and one dies, with the behavior hardwired in a way they could not predict from the rules alone. We have a compendium where we can run their test on thousands of creatures and check whether the swervers form a connected region of the morphospace. If they do, the map is predicting behavior it was never built to measure.
