...
The improved timings showed that the revised query was fast enough for a full test; they did not show that the source archive was safe to delete. That required the complete run to finish, the resulting database to pass its checks and confirmation that no process was still writing to either copy. The interrupted run remained useful as a diagnosis, while the original archive remained necessary. By the end of this period, the replacement path was ready for serious testing and the 281 GB source was still intact.
